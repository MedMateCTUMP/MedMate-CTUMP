...
const initDb = () => {
  if (!fs.existsSync(DB_FILE)) {
    const initialData = {
      ...
      otps: {} as Record<string, { otp: string; expires: number }>
    };
...
app.post("/api/subjects", (req, res) => {
  ...
  if (id) {
    // Edit
    const index = db.subjects.findIndex((s: any) => s.id === id);
    if (index !== -1) {
...
app.delete("/api/subjects/:id", (req, res) => {
  ...
  db.sessions = db.sessions.filter((s: any) => s.subjectId !== id);
  writeDb(db);
  ...
  if (id) {
    ...
    if (index !== -1) {
      ...
      if (oldSession.status !== "done" && sessionObj.status === "done") {
        ...
        const userEmail = req.headers["x-user-email"] as string || "dd0369797@gmail.com";
...
app.post("/api/screentime", (req, res) => {
  ...
  const db = readDb();
  if (!db.screenTime) db.screenTime = [];

  const todayStr = new Date().toISOString().split("T")[0];

  // See if there's already an entry for this app today, if so we add to it
  const existingIndex = db.screenTime.findIndex((st: any) => st.appName.toLowerCase() === appName.toLowerCase() && st.date === todayStr);
...
app.delete("/api/screentime/:id", (req, res) => {
  const { id } = req.params;
  const db = readDb();
  db.screenTime = db.screenTime.filter((st: any) => st.id !== id);
  writeDb(db);
  res.json({ success: true, screenTime: db.screenTime });
});
...
app.post("/api/limits", (req, res) => {
  ...
  const db = readDb();
  if (!db.appLimits) db.appLimits = [];

  if (id) {
    const index = db.appLimits.findIndex((l: any) => l.id === id);
    if (index !== -1) {
...
app.delete("/api/limits/:id", (req, res) => {
  const { id } = req.params;
  const db = readDb();
  db.appLimits = db.appLimits.filter((l: any) => l.id !== id);
  writeDb(db);
  res.json({ success: true, appLimits: db.appLimits });
});
...
Hãy viết câu trả lời ngắn gọn, rành mạch, có chia các gạch đầu dòng rõ ràng, tuyệt đối không viết một khối văn bản dài dằng dặc.`;
  ...
  if (!client) {
    ...
    if (cleanQuery.includes("hôm nay học gì") || cleanQuery.includes("lich hoc") || cleanQuery.includes("học gì")) {
      const todaySessions = sessionsList.filter((s: any) => s.startTime.startsWith(new Date().toISOString().split("T")[0]));
